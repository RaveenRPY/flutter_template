...
class AppValidator {
  ...
  DateResponse getDateResponse(String nic) {
    ...
    dateResponse.month =
        i + 1; // Adding 1 to convert from 0-based index to 1-based month
    dateResponse.day = day;

    return dateResponse;
  }

  static NicDetailsBean calculateNicDetails(String nic) {
    RegExp regex = RegExp(r'^(\d{9}[xXvV]|\d{12})$');

    if (!regex.hasMatch(nic)) {
      throw Exception('Invalid NIC: $nic');
    }
    ...
    // Extract Day and Year from NIC
    bool isNewFormat = nic.length == 12;
    if (isNewFormat) {
      day = int.parse(nic.substring(4, 7));
      y = int.parse(nic.substring(0, 4));
    } else {
      day = int.parse(nic.substring(2, 5));
      y = 1900 + int.parse(nic.substring(0, 2));
    }
    ...
    m = i + 1;
    retrievedDate = '$y-$m-$d';
    DateTime date = DateFormat('yyyy-MM-dd').parse(retrievedDate);
    print(date);

    if (m > 12 || d > 31) {
      throw Exception(
          'Invalid NIC date===========================================: $retrievedDate');
    }

    int age = calculateAge(date);

    return NicDetailsBean(
  ...
  static int calculateAge(DateTime dob) {
    ...
    return curDate.year -
        dob.year -
        ((curDate.month > dob.month ||
            (curDate.month == dob.month && curDate.day >= dob.day))
            ? 0
            : 1);
    }
  ...
  static bool validateMobileNumber(String mobileNo) {
  ...
  bool isValidMobileNumberOnFreshLogin(String mobile) {
   ...
   bool _isUppercaseValidated(String word, int? count) {
    if (count == 0) {
      return true;
    } else {
      int matches = 0;
  ...
  bool _isLowercaseValidated(String word, int? count) {
    if (count == 0) {
      return true;
    } else {
      int matches = 0;
   ...
   bool _isNumericValidated(String word, int? count) {
    if (count == 0) {
      return true;
    } else {
      int matches = 0;
   ...
   bool isLowerCase(String character) {
    if (character.isEmpty) return false;
  ...
  bool isSpecialCharacter(String character) {
    if (character.isEmpty) return false;
    return RegExp(r'[!@#$%^&*(),.?":{}|<>]').hasMatch(character);
  }
}
...
class ValidateState {
  ...
  ValidateState({this.validate, this.description});
}

class NicDetailsBean {
  String? nic;
  String? gender;
  String? birthdate;
  int? age;
  ...
  NicDetailsBean({this.nic, this.gender, this.birthdate, this.dobRaw, this.age});
}
